...
set_path = r"\\Dmitrienko-1064.sl.iae.nsk.su\share\set_4"
def run(wave_json):
    db = FileDatabase()
    share_folder = 'share'
    ...
    wave_info = json.loads(wave_json)
    wave = fr'{set_path}\basises\{wave_info["basis"]}\{wave_info["file"]}'
    ...
    db.save_files(wave_info["basis"],wave_info["file"], f"dat.txt", f"out.nc")
...
def process(info_file_path,bath_file_path,wave,  wave_file_path, data_folder):
    db = FileDatabase()
    share_folder = 'share'
    bathes = get_files_with_extension(set_path, "bath")
    bath = bathes[0]
